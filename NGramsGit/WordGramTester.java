...
public class WordGramTester {
	...
	public void testMyWordGram(){
		String source = "this is a test this is a test this is a test of words";
		String[] words = source.split("\\s+");
		int size = 4;
		for(int index = 0; index <= words.length - size; index += 1) {
			MyWordGram wg = new MyWordGram(words,index,size);
			System.out.println(index+"\t"+wg.length()+"\t"+wg);
		}
	}
	public void testMyWgShiftAdd(){
	   String source = "this is a test this is a test this is a test of words";
		String[] words = source.split("\\s+");
		int size = 4;
		MyWordGram wg = new MyWordGram(words, 0, size);
		out.println(wg);
		MyWordGram newWg = wg.shiftAdd("absolutely");
		out.println(newWg);
	   }
	/*
	...
	*/
	public void testMyWordGramEquals(){
		String source = "this is a test this is a test this is a test of words";
		String[] words = source.split("\\s+");
		ArrayList<MyWordGram> list = new ArrayList<MyWordGram>();
		int size = 4;
		for(int index = 0; index <= words.length - size; index += 1) {
			MyWordGram wg = new MyWordGram(words,index,size);
			list.add(wg);
		}
		MyWordGram first = list.get(0);
		System.out.println("checking "+first);
		for(int k=0; k < list.size(); k++){
			//if (first == list.get(k)) {
			  if (first.equals(list.get(k))) {
